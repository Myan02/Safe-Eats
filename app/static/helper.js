...
export async function updateZipcodes() {
    try {
        ...
        const response = await fetch("/inspections/average");

        if (!response.ok) {
            throw new Error("Couldn't get average grades per zipcode from helper.js...");
        }
        const zipcode_entries = await response.json();

        // set up the initial properties of the match expression, we want to match each zipcode (hence modzcta) with a color
...
export async function getUniqueRestaurants() {
    ...
    try {
        ...
        const response = await fetch("/inspections/unique");
        
        if (!response.ok) {
            throw new Error("Couldn't get all unique restaurants from helper.js...");
        }
